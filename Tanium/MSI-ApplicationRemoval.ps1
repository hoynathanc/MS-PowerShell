...
# Decode any URL-encoded characters passed by Tanium
...
switch ($SearchType) {
    "Exact" {
        $InstalledApps = Get-App -Product $Application -Exact
    }
    "Wildcard" {
        $InstalledApps = Get-App -Product $Application -WildCard
    }
    "Regex" {
        $InstalledApps = Get-App -Product $Application -RegEx
    }
    default {
        $InstalledApps = Get-App -Product $Application
    }
}
...
foreach ($app in $InstalledApps) {
    if ($app.UninstallString -like "MsiExec*") {
        ...
        Write-Host "Application Found: $App"
